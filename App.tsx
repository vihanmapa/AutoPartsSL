...
const CartView: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      {/* Header handled by MobileHeader */}
...
const AppContent: React.FC = () => {
  const { view, isLoading, loadingMessage } = useApp();
  ...
  // Views that don't need Navbar/BottomNav
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-900 bg-slate-50">
      {/* Mobile Header for Buyer Views */}
      {isBuyerView && <MobileHeader />}

      {/* Desktop Navbar (Hidden on mobile via CSS if needed, or conditional rendering) */}
      ...
      {isBuyerView && <BottomNav />}

      {/* Footer only for non-buyer/non-vendor views or desktop specific */}
      {!isBuyerView && view !== 'vendor' && (
